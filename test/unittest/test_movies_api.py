...
@patch("movie_api.requests.get")
def test_get_movie_by_api_id(mocked_get, mocked_movie_api):
    m = MagicMock()
    m.status_code = 200
    m.json.return_value = {"movie_id": "123"}
    mocked_get.return_value = m

    ret = mocked_movie_api.get_movie_by_api_id("tmdb", "123", "TEST_TOKEN")

    assert ret == {"movie_id": "123"}


@patch("movie_api.requests.get")
def test_get_movie_by_api_id_invalid_code(mocked_get, mocked_movie_api):
    m = MagicMock()
    m.status_code = 404
    mocked_get.return_value = m
