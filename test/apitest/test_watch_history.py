...
def test_get_watch_history_invalid_auth():
    res = requests.get(
        f"{API_URL}/watch-history", headers={"Authorization": "Invalid"}
    )
...
def test_get_watch_history_invalid_collection_item():
    res = requests.get(
        f"{API_URL}/watch-history/collection/invalid/123", headers=BASE_HEADERS
    )
...
def test_get_watch_history():
    ...
    assert (
        item_res["items"]["23d5d8c1-2ab0-5279-a501-4d248dc9a63c"][
            "collection_name"
        ]
        == "anime"
    )
    assert (
        "created_at"
        in item_res["items"]["23d5d8c1-2ab0-5279-a501-4d248dc9a63c"]
    )
    assert (
        "updated_at"
        in item_res["items"]["23d5d8c1-2ab0-5279-a501-4d248dc9a63c"]
    ...
    assert (
        item_res["items"]["cad1a63b-d876-5833-a5db-8cd33f4c92ec"][
            "collection_name"
        ]
        == "anime"
    )
    assert (
        "created_at"
        in item_res["items"]["cad1a63b-d876-5833-a5db-8cd33f4c92ec"]
    )
    assert (
        "updated_at"
        in item_res["items"]["cad1a63b-d876-5833-a5db-8cd33f4c92ec"]
